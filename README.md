# Рыбаки — региональный чат и маркетплейс (MVP-каркас)
...
2. Authentication → Providers → **Phone** → включить, подключить
   SMS-провайдера (Twilio / MessageBird / Vonage и т.п.) — это делается
   в дашборде, SQL-скрипт этот шаг не покрывает.
3. Выполнить `supabase/schema.sql` в SQL Editor — создаст таблицы
...
4. Выполнить `supabase/seed.sql` — добавит регионы «Москва» и
   «Санкт-Петербург» и несколько тестовых товаров каталога (скрипт
   идемпотентен, можно запускать повторно).
5. Взять `Project URL` и `anon public key` из Project Settings → API.
6. В Database → Replication включить Realtime для таблицы `messages`
